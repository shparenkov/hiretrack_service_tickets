...
class InMemoryTicketStore {
    ...
    constructor(filePath = null) {
        ...
        const stored = JSON.parse(fs_1.default.readFileSync(filePath, 'utf8'));
        this.sequence = Number.isInteger(stored.sequence) ? Number(stored.sequence) : 0;
        this.tickets = new Map((stored.tickets || []).map((ticket) => [ticket.id, ticket]));
        this.activityLogs = new Map(Object.entries(stored.activityLogs || {}));
    }
    async listTickets() {
    ...
    persist() {
        ...
        const activityLogs = Object.fromEntries(this.activityLogs.entries());
        ...
        fs_1.default.writeFileSync(temporaryPath, payload, 'utf8');
        if (fs_1.default.existsSync(this.filePath)) {
            fs_1.default.rmSync(this.filePath);
        }
        fs_1.default.renameSync(temporaryPath, this.filePath);
    }
}
...
function createTicketStore() {
    ...
    if (mode === 'file') {
        const filePath = path_1.default.resolve(process.env.TICKETS_FILE_PATH || path_1.default.join(process.cwd(), 'data', 'tickets.json'));
        return new InMemoryTicketStore(filePath);
    }
