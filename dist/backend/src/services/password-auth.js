...
function sessionTtlMs() {
    const configuredDays = Number(process.env.SERVICE_TICKETS_SESSION_DAYS || DEFAULT_SESSION_DAYS);
...
function readPassword() {
    const directPassword = process.env.SERVICE_TICKETS_ACCESS_PASSWORD?.trim();
    if (directPassword)
        return directPassword;
    const passwordFile = process.env.SERVICE_TICKETS_ACCESS_PASSWORD_FILE?.trim();
    if (!passwordFile)
        return '';
    try {
...
<head>
  ...
  <title>Сервисные тикеты · Вход</title>
  <style>
